#!/usr/bin/env python3
"""Transformer — batch 2048, 6 layers, 8 heads"""
...
from models_v2 import DeepTransformer
...
device="cuda"
m=DeepTransformer(Xt.shape[2],256,8,6,96).to(device)
...
for ep in range(100):
    ...
    if acc>best:best=acc;pat=0;torch.save({"state":m.state_dict(),"val_acc":acc},f"{BASE}/training/checkpoints/transformer_best.pth")
    else:pat+=1
    vram=torch.cuda.memory_allocated(0)/1024/1024/1024
    print(f"TFR EP{ep:>3} | VL:{vl:.4f} | Acc:{acc:.3f} | Best:{best:.3f} | VRAM:{vram:.2f}GB | {time.time()-t0:.0f}s",flush=True)
    if pat>=15:print(f"Early stop EP{ep}",flush=True);break
print(f"Transformer Done: Best={best:.3f}",flush=True)